...
PixelIn vs_main(VertexIn i)
{
	...
	o.uv.x += RDS_TEXTURE_2D_SAMPLE_LOD(texture1, i.uv, 0).r;
	//o.uv.x += RDS_TEXTURE_2D_SAMPLE_LOD(texture0, i.uv, 0).r;

	 o.uv.x += RDS_RW_BUFFER_LOAD_I(TestBuffer, testRwBuffer, 0).color.r;
	// o.uv.x += RDS_RW_BUFFER_LOAD_I(TestBuffer, testRwBuffer, 1).color.g;
	// o.uv.x += RDS_RW_BUFFER_LOAD_I(TestBuffer, testRwBuffer, 2).color.b;
	// o.uv.x += RDS_RW_BUFFER_LOAD_I(TestBuffer, testRwBuffer, 3).color.a;
	// o.uv.x += RDS_BUFFER_LOAD_I(TestBuffer, testBuffer, 0).color.r;
	// o.uv.x += RDS_BUFFER_LOAD_I(TestBuffer, testBuffer, 1).color.g;
	// o.uv.x += RDS_BUFFER_LOAD_I(TestBuffer, testBuffer, 2).color.b;
	// o.uv.x += RDS_BUFFER_LOAD_I(TestBuffer, testBuffer, 3).color.a;
...
float4 ps_main(PixelIn i) : SV_TARGET
{
	...
	color += RDS_TEXTURE_2D_SAMPLE(texture1, i.uv);
