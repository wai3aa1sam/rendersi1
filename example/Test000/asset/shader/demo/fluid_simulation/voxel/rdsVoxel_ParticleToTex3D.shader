...
#include "../3d/rdsFluidSim3D_Common.hlsl"
...
float2 calcDensityDataAtPoint(float3 samplingPt, float radius, uint elemntCount)
{
	float density = 0;
	float nearDensity = 0;
...
void Cs_particleToTex3D(ComputeIn input)
{
	...
	//tex[id] = calcParticleCountAtPoint(posWs, u_smoothingRadius, u_particleCount) / (half)u_particleCount;
	tex[id] = calcDensityDataAtPoint(posWs, u_smoothingRadius, u_particleCount).x;
}
