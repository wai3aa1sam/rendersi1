...
#include "rdsWavefrontObjLoader.h"

namespace rds 
{

void 
WavefrontObjLoader::loadFile(EditMesh& mesh, StrView filename) 
{
	MemMapFile mm;
	mm.open(filename);
	loadMem(mesh, mm);
...
void 
WavefrontObjLoader::loadMem(EditMesh& mesh, ByteSpan src) 
{
...
void 
WavefrontObjLoader::_loadMem(EditMesh& mesh, ByteSpan src) 
{
	mesh.clear();
	_outMesh = &mesh;
	_outMesh->primitive = RenderPrimitiveType::Triangle;

	_source = makeStrView(src);
...
void 
WavefrontObjLoader::_nextLine() 
{
	auto ret = StrUtil::splitByChar(_sourceRemain, "\n");
...
{
	auto ret    = StrUtil::splitByChar(_lineRemain, " \t\r");
	_token      = StrUtil::trimChar(ret.first,		" \t\r");
	_lineRemain = StrUtil::trimChar(ret.second,		" \t\r");
}
...
void WavefrontObjLoader::_parseLine_v() {
	...
	for (size_t i = 0; i < 4; i++) {
		...
		if (!StrUtil::tryParse(_token, value.data[i])) 
		{
			_error("error read v");
		}
	}
...
void WavefrontObjLoader::_parseLine_f() {
	...
	while (_lineRemain.size() > 0) 
	{
		...
		auto pair = StrUtil::splitByChar(_token, '/');
		if (!StrUtil::tryParse(pair.first, vi)) 
		{
		...
		if (pair.first.size()) 
		{
			if (!StrUtil::tryParse(pair.first, vt)) 
			{
		...
		if (pair.first.size()) 
		{
			if (!StrUtil::tryParse(pair.first, vn)) 
			{
	...
	for (int i = 0; i < face_vi.size(); i++) {
		...
		if (face_vt.size() >= face_vi.size()) {
			...
			_outMesh->uvs[0].emplace_back(_tmpUv[vt]);
		}
	...
	for (size_t i = 0; i < face_vi.size(); i++)
	{
		if (i >= 3) 
		{
			_outMesh->indices.emplace_back(sCast<u32>(indicesStart + 0));
			_outMesh->indices.emplace_back(sCast<u32>(indicesStart + i - 1));
		}

		u32 vi = static_cast<u32>(indicesStart + i);
		if (vi >= _outMesh->pos.size()) 
		{
			_error("vi out of range");
		}
