...
Vk_DescriptorAllocator::alloc(Vk_DescriptorSet* oSet, const Vk_DescriptorSetLayout* layout, bool checkValid)
{
	...
	RDS_RENDER_VK_SET_DEBUG_LABEL(out, RDS_DebugLabel(), renderDeviceVk());
}
...
Vk_DescriptorAllocator::requestPool()
{
	...
	else
	{
		...
		RDS_RENDER_VK_SET_DEBUG_LABEL(*out, RDS_DebugLabel("VkDescPools[{}]", _pools.size()), renderDeviceVk());
	}
...
void 
Vk_DescriptorAllocator::setDebugLabel(RDS_DebugLabel_PARAM)
{
